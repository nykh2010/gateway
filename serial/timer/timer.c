...
char * time_to_string(char * str, time_t t) {
	struct tm *ptm;
	ptm = gmtime(&t);
	snprintf(str, 20, "%4d-%02d-%02d %02d:%02d:%02d", ptm->tm_year+1900, ptm->tm_mon+1, ptm->tm_mday, ptm->tm_hour, ptm->tm_min, ptm->tm_sec);
	return str;
}

int system_time_to_string (char * str, struct tm_utc * tm) {
	snprintf(str, 20, "%4d-%02d-%02d %02d:%02d:%02d", tm->tm_year, tm->tm_mon, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);
	return 0;
}
