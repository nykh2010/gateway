...
function wireless_security_ap_change(val,type) {
    ...
    if (val == "0") {
        ...
        reg_map["wireless_base_"+ type +"_frm"][6].type = "password eq8_63";
    }
}
...
function mywireless_base_set(type) {
	var obj = {};
		...
		obj.ssid = $("#wire_"+ type +"_ssid").val();
		obj.lbl = $("#wls_"+type+"_ap_mode_sel_1").val();
		obj.key = $("#wireless_"+type+"_key_val").val();
		if (obj.key.length < 8) {
...
function wireless_base_set(type,wirelessSetCallback) {
   if (check_input("wireless_base_"+ type +"_frm")) {
		...
		if(change){
			//发送原来的cgi
			show_message("save");
			$.post("/setup/wifi", obj, function (data) {
				var data = dataDeal(data);
				if (data == "SUCCESS") {
					obj2._xsrf = xsrf_tmp;
					$.post("/setup/wifi", obj2, function (data2) {
						var data2 = dataDeal(data2);
						if (data2 == "SUCCESS") {
							if(type == "2_4"){
...
function init_wireless_advance_show(){
	$.post("/router/wireless_ap_adv_op.cgi", {action: "0"}, function (data) {
		...
		radio_set(data.adaptivity_enable,'power_detect');
	});
}
